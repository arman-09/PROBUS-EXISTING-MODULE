...
class ManagementReporter:
    ...
    def _is_monitored(self, alert: dict) -> bool:
        ...
        if "BUS COUPL" in feeder or alert.get("IsBusCoupler"):
            return False
...
<div style="background:#fff;border:1px solid #e8ecf0;border-left:4px solid {color};border-radius:6px;padding:12px 14px;margin:8px 12px">
  ...
  {div_note_card}
        ...
        for circ, d in sorted(circles.items()):
            mw  = round(d["MW"], 3)
            mva = round(d["MVA"], 3)
            pk  = round(daily.get(circ, {}).get("peak_mw", mw), 3)
            total_mw += mw
            circle_rows += f"""
<tr>
  <td style="padding:7px 12px;border-bottom:1px solid #e8ecf0;font-weight:600">{circ}</td>
  <td style="padding:7px 12px;border-bottom:1px solid #e8ecf0;text-align:right;font-family:monospace">{mw:.3f}</td>
  <td style="padding:7px 12px;border-bottom:1px solid #e8ecf0;text-align:right;font-family:monospace">{mva:.3f}</td>
  <td style="padding:7px 12px;border-bottom:1px solid #e8ecf0;text-align:right;font-family:monospace;color:#0066cc">{pk:.3f}</td>
</tr>"""
...
<tr><td style="background:#fff;padding:0">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px">
    ...
    <tfoot><tr style="background:#e9ecef;font-weight:700">
      ...
      <td style="padding:10px 12px;text-align:right;font-family:monospace">{total_mw:.3f}</td>
      <td colspan="2"></td>
    </tfoot>
  </table>
</td></tr>
