    ...
    def update(self, live_data: list, off_thr: float = 1.0) -> list:
        ...
        for gss, rows in by_gss.items():
            ...
            for bc in bcs:
                ...
                bc_just_restored = prev_b <= off_thr and bcimax > off_thr

                if diverted_via_this_bc:
                    bc_trip_ak = f"{bac}_{VT_BCT}"
                    if bc_just_tripped:
                        if not self._store_ref.is_active(bc_trip_ak):
                            for fac2, d in diverted_via_this_bc:
                                ...
                                if self._store_ref.add(v, None):
                                    ...
                                    log.warning(f"BC_TRIP_WHILE_DIVERTED: {bac} tripped "
                                                ...
                                                f"diverted load @ {gss}")
                    elif bc_just_restored or not bc_is_active:
                        pass  # handled in the restoration check below

                    # Restoration of the BC-trip-while-diverted condition:
                    # fires when BC is back ON (current restored through it)
                    # — per spec this auto-acknowledges, mirroring LOAD_RESTORED.
                    if bc_is_active and self._store_ref.is_active(bc_trip_ak):
                        for fac2, d in diverted_via_this_bc:
                            fdr2 = next((f for f in fdrs
                                         if f.get("AssetCode","") == fac2), None)
                            fdr_name = fdr2.get("Feeder", fac2) if fdr2 else fac2
                            dur = self._store_ref.get_active_duration(bc_trip_ak)
                            ...
                            rv = {
                                ...
                                "lim":       off_thr,
                                "detail":    (f"Current: {bcimax:.1f}A (Restored)."
                                              + (f" Normalized after: {dur_str}." if dur_str else "")),
                                "timestamp": now_iso,
                                "first_seen": now_iso,
                            ...
                            if self._store_ref.add(rv, None):
                                viols.append(rv)
                                log.info(f"BC_DIVERTED_NORMALIZED: {bac} restored "
                                         f"({bcimax:.1f}A), diverted feeder {fac2} "
                                         f"({fdr_name}) supply back. Was OFF {dur_str}")
