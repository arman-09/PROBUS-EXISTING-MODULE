...
def _index_blocks(blocks: list, date_str: str) -> dict:
    ...
    FIX — duplicate-slot collision (confirmed on Lalbazar PSS's real
    exports): some sites publish TWO entries for the exact same 15-min
    slot — the real reading, immediately followed by an all-zero stub
    for that same timestamp. A naive "last one wins" dict build picks
    whichever happens to be LAST in the array, which for Lalbazar is
    always the zero stub — that's exactly what was making Baripada (via
    Lalbazar PSS) show "—" for every field even though real data existed
    for every slot. When two entries collide on the same slot key, keep
    whichever has the HIGHER recordingCount (i.e. the one that actually
    has real samples behind it), not whichever came last.
    """
    idx = {}
    for b in blocks:
        try:
            key = (date_str, int(b["scheduleHour"]), int(b["scheduleMinuteLower"]))
        except (KeyError, TypeError, ValueError):
            continue
        existing = idx.get(key)
        if existing is None or (b.get("recordingCount") or 0) > (existing.get("recordingCount") or 0):
            idx[key] = b
    return idx
