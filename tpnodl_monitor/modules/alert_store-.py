...
STATEFUL = {"OV", "UV", "OL", "FEEDER_OFF", "LOAD_DIVERTED", "PT_PHASE_MISSING",
            "LINE_JUMPER_PARTING", "BC_TRIP_WHILE_DIVERTED"}
...
class AlertStore:
    ...
    def add(self, alert: dict, cfg=None) -> bool:
        ...
        # Determine which types are latched (stateful)
        # Latched = the type is in the configured alert_types (checked in UI)
        # If alert_types is None/empty (all checked), latch only the core stateful types
        if cfg:
            alert_types = cfg.get("email.alert_types") or []
            latched_types = set(alert_types) if alert_types else STATEFUL
        else:
            latched_types = STATEFUL
    ...
    def _insert(self, a: dict):
        ...
        INSTANT_TYPES = {"LOAD_RESTORED", "SUDDEN_LOAD_DROP", "SUDDEN_LOAD_RAISE",
                          "BC_LOADING_START", "BC_DIVERTED_NORMALIZED"}
