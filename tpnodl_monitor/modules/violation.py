...
VT_BCN = "BC_RESUMED_DIVERSION"       # PREVIOUSLY MISSING — the BC's OWN
                                       # current resumes (the BC trip itself
                                       # has cleared) WHILE the originally-
                                       # diverted feeder is STILL off. This
                                       # does NOT clear VT_BCT — the feeder's
                                       # own outage is the real unresolved
                                       # problem and is tracked by VT_BCT/
                                       # its own FEEDER_OFF alert exactly as
                                       # before. This is a separate, one-time
                                       # informational ping: "the no-backup-
                                       # path emergency has eased, the BC is
                                       # carrying the diversion again — but
                                       # the original feeder still needs
                                       # attention." Fires once per BCT
                                       # incident (flagged via extra_json so
                                       # it doesn't repeat every cycle while
                                       # the BC stays up).
VT_COMM = "COMM_DOWN"                 # Meter/feeder communication down —
                                       # data not received for 2+ consecutive
                                       # scan cycles. Stays STATEFUL until
                                       # the meter responds again, then
                                       # auto-clears with a normalization
                                       # event. NEVER appears in the
                                       # Management Report — dashboard and
                                       # instant WA/email alerts only.
VT_COMM_RST = "COMM_RESTORED"         # Normalization: meter communication
                                       # came back online. INSTANT_TYPE —
                                       # never latched, auto-acked like
                                       # LOAD_RESTORED.
...
SEVERITY = {
    ...
    VT_BCN:"INFO",       # BC resumed diversion — informational, feeder
                          # itself is still tracked separately as CRITICAL
    VT_COMM:"HIGH",      # Meter communication down — high priority,
                          # could mask any real underlying violation
    VT_COMM_RST:"INFO",  # Communication restored — informational
}
STATEFUL = {VT_OV, VT_UV, VT_OL, VT_PTF, VT_LJP, VT_BCT, VT_COMM}


class ViolationDetector:
    def __init__(self, cfg, feeder_master, alert_store, email_mgr, wa_mgr,
                 peak_store=None, notify_queue=None):
        self.cfg        = cfg
        self.fm         = feeder_master
        # Optional persistent retry queue (see notify_queue.py). When
        # None (default — fully backward compatible), _notify() below
        # sends directly exactly as it always has. When provided, sends
        # are enqueued instead, gaining automatic retry/backoff for
        # transient failures (Gmail rate limits, a busy WA driver, etc.)
        # that previously meant permanent, silent notification loss.
        ...
        self._ptf_cycles: dict   = {}  # asset -> consecutive PTF data cycles
        self._comm_miss:  dict   = {}  # asset -> consecutive missing scan count
        self._comm_first: dict   = {}  # asset -> datetime of first missing scan
        self._frozen_cycles: dict = {} # asset -> consecutive cycles with no change in any of Vr/Vy/Vb/Ir/Iy/Ib
        self._frozen_last:   dict = {} # asset -> last snapshot tuple (Vr,Vy,Vb,Ir,Iy,Ib)
        self._frozen_first:  dict = {} # asset -> datetime frozen detection started
    ...
    def run(self, live_data: list) -> list:
        ...
        for row in live_data:
            ...
            if not is_bc and vavg > 1.0 and imax > off_thr:
                ak_ptf = f"{asset}_{VT_PTF}"
                ptf_active = ak_ptf in self.store._active
                ...
                if v_unbal_pct > 25.0:
                    ...
                    if self.store.add(v, self.cfg):
                        raw_viols.append(v)
                    self._ptf_cycles[asset] = self._ptf_cycles.get(asset, 0) + 1
                else:
                    # PTF condition cleared — create a normalization event,
                    # exactly like LJP normalization, so ops know the PT
                    # fault has self-resolved and the feeder voltage is
                    # balanced again — previously this was a silent
                    # clear_condition() with no notification at all.
                    if ptf_active:
                        cycles = self._ptf_cycles.get(asset, 0)
                        if cycles <= 1:
                            label = "PT Phase Restored (Transient)"
                        else:
                            label = "PT Phase Restored"
                        ptf_norm = self._make(VT_PTF, row, master,
                            value=round(v_unbal_pct, 2), limit=25.0,
                            detail=(f"{label}: Voltage balance restored. "
                                    f"Vr/Vy/Vb: {vr:.3f}/{vy:.3f}/{vb:.3f} kV "
                                    f"(Vavg={vavg:.3f} kV, unbalance={v_unbal_pct:.1f}%). "
                                    f"Was active for {cycles} data cycle(s)."))
                        ptf_norm["_ptf_norm"]  = True
                        ptf_norm["_ptf_false"] = (cycles <= 1)
                        ptf_norm["acked"]       = True  # auto-ack like LOAD_RESTORED
                        raw_viols.append(ptf_norm)
                        self._ptf_cycles.pop(asset, None)
            ...
            if not is_bc and imax > off_thr and master:
                ...
                if ljp_condition:
                    ...
                    if ljp_active:
                        ...
                        ljp_norm["acked"]       = True  # auto-ack like LOAD_RESTORED
            ...
            # ── Frozen-data COMM_DOWN detection ───────────────────────────
            # If all 6 parameters (Vr, Vy, Vb, Ir, Iy, Ib) are exactly
            # identical to the last recorded snapshot for N consecutive
            # cycles (default 5 × 2min = 10min), the meter is likely
            # frozen — it's technically sending data but the SCADA system
            # is caching the last known value rather than live readings.
            # This is a separate sub-type of COMM_DOWN from the "meter
            # absent" variant above, distinguished by asset_key suffix
            # so both can track independently without collision.
            if not is_bc:
                frozen_threshold = self.cfg.get("voltage.frozen_cycles_threshold", 5)
                ak_frozen = f"{asset}_COMM_DOWN_FROZEN"
                snap = (vr, vy, vb, ir, iy, ib)
                last_snap = self._frozen_last.get(asset)

                all_same = (last_snap is not None and snap == last_snap)
                if all_same:
                    self._frozen_cycles[asset] = self._frozen_cycles.get(asset, 0) + 1
                else:
                    # Values changed — clear frozen alert if active
                    if self.store.is_active(ak_frozen):
                        comm_dur = self.store.get_active_duration(ak_frozen)
                        dur_str  = _fmt_dur_sec(comm_dur) if comm_dur else ""
                        frst_first = self._frozen_first.get(asset, "")
                        frst_norm = {
                            "id":           f"{asset}_{VT_COMM_RST}_frz_{int(time.time()*1000)}",
                            "asset_key":    f"{asset}_{VT_COMM_RST}_frozen",
                            "type":         VT_COMM_RST,
                            "severity":     "INFO",
                            "AssetCode":    asset,
                            "Circle":       row.get("Circle",""),
                            "Division":     row.get("Division",""),
                            "Gss":          row.get("Gss",""),
                            "Feeder":       master.get("FeederName","") or row.get("Feeder", asset),
                            "FeederType":   row.get("FeederType",""),
                            "IsBusCoupler": False,
                            "FeederRating": master.get("FeederRating"),
                            "Vr":vr,"Vy":vy,"Vb":vb,
                            "Ir":ir,"Iy":iy,"Ib":ib,
                            "ApparentPower":None,
                            "value":        0, "limit": frozen_threshold,
                            "detail":       (f"Frozen data cleared: meter readings "
                                             f"resumed changing after {dur_str or '?'}. "
                                             f"New values — Vr={vr:.3f} Vy={vy:.3f} "
                                             f"Vb={vb:.3f} kV  Ir={ir:.1f} Iy={iy:.1f} "
                                             f"Ib={ib:.1f} A"),
                            ...
                            "notified_wa":    False,
                            "_comm_norm":         True,
                            "_comm_duration_str": dur_str,
                            "_comm_frozen":       True,
                        }
                        self.store.clear_condition(asset, "COMM_DOWN_FROZEN")
                        raw_viols.append(frst_norm)
                        log.info(f"COMM_DOWN_FROZEN cleared: {asset} — "
                                 f"data changed after {dur_str or '?'}")
                    self._frozen_cycles.pop(asset, None)
                    self._frozen_first.pop(asset, None)

                self._frozen_last[asset] = snap

                frz_count = self._frozen_cycles.get(asset, 0)
                if all_same and frz_count == frozen_threshold:
                    # Fire the frozen alert exactly once at threshold
                    frz_first = self._frozen_first.get(asset, datetime.now().isoformat())
                    feeder_name = master.get("FeederName","") or row.get("Feeder", asset)
                    frz_v = {
                        "id":           f"{asset}_COMM_DOWN_FROZEN_{int(time.time()*1000)}",
                        "asset_key":    ak_frozen,
                        "type":         VT_COMM,
                        "severity":     SEVERITY.get(VT_COMM,"HIGH"),
                        "AssetCode":    asset,
                        "Circle":       row.get("Circle",""),
                        "Division":     row.get("Division",""),
                        "Gss":          row.get("Gss",""),
                        ...
                        "ApparentPower":None,
                        "value":        frz_count,
                        "limit":        frozen_threshold,
                        "detail":       (f"Frozen data detected: no change in any of "
                                         f"Vr/Vy/Vb/Ir/Iy/Ib for {frz_count} consecutive "
                                         f"scan cycles (~{frz_count*2}min). "
                                         f"Stuck values — Vr={vr:.3f} Vy={vy:.3f} "
                                         f"Vb={vb:.3f} kV  Ir={ir:.1f} Iy={iy:.1f} "
                                         f"Ib={ib:.1f} A. "
                                         f"Communication Down / Meter OFF likely."),
                        "timestamp":    datetime.now().isoformat(),
                        "acked":        False,
                        "notified_email": False,
                        "notified_wa":    False,
                        "_comm_first_missing": frz_first,
                        "_comm_miss_count":    frz_count,
                        "_comm_frozen":        True,
                    }
                    if self.store.add(frz_v, self.cfg):
                        raw_viols.append(frz_v)
                        log.info(f"COMM_DOWN_FROZEN: {asset} ({feeder_name}) — "
                                 f"no change in Vr/Vy/Vb/Ir/Iy/Ib for "
                                 f"{frz_count} cycles ({frz_count*2}min)")
                elif all_same and frz_count == 1:
                    self._frozen_first[asset] = datetime.now().isoformat()

        # ── Communication Down detection ──────────────────────────────────
        # Compare assets seen this cycle against the full feeder master.
        # If a meter is expected (in feeder master, not a Bus Coupler)
        # but absent from live_data for 2+ consecutive scans, create a
        # COMM_DOWN alert. Clears automatically with a normalization event
        # when the meter responds again.
        # NEVER included in the Management Report (see REPORT_TYPES in
        # mgmt_report.py) — dashboard and instant WA/email alerts only.
        seen_this_cycle = {(r.get("AssetCode") or "").strip()
                           for r in live_data
                           if (r.get("AssetCode") or "").strip()}
        comm_miss_threshold = self.cfg.get("voltage.comm_miss_threshold", 2)

        # BULK-FAILURE GUARD: if the scraper itself fails to reach the
        # SCADA system for a cycle, every meter vanishes simultaneously —
        # this would fire COMM_DOWN for 100+ feeders at once. The tell:
        # all first-missing timestamps share the same second (confirmed
        # in live data: 30 alerts all timestamped 08:43:27.714xxx).
        # If >50% of expected non-BC feeders are absent this cycle,
        # treat it as a SCADA connectivity blip and skip COMM_DOWN
        # creation entirely — individual miss counters are NOT incremented
        # (the counters reset cleanly on the next successful fetch).
        all_monitored = [f for f in self.fm.all()
                         if (f.get("AssetCode") or "").strip()
                         and not f.get("IsBusCoupler")
                         and "BUS COUPL" not in (f.get("FeederName","") or "").upper()
                         and "BUS COUPL" not in (f.get("Feeder","") or "").upper()]
        expected_count = len(all_monitored)
        absent_count   = sum(1 for f in all_monitored
                             if (f.get("AssetCode") or "").strip() not in seen_this_cycle)
        scada_blip = (expected_count > 0 and
                      absent_count / expected_count > 0.50)
        if scada_blip:
            log.warning(f"COMM_DOWN: {absent_count}/{expected_count} meters absent "
                        f"this cycle ({absent_count/expected_count:.0%}) — "
                        f"treating as SCADA connectivity blip, suppressing "
                        f"individual COMM_DOWN alerts. Miss counters NOT incremented.")

        for fdr_info in all_monitored:
            fdr_asset = (fdr_info.get("AssetCode") or "").strip()
            ak_comm = f"{fdr_asset}_{VT_COMM}"

            if fdr_asset not in seen_this_cycle:
                if scada_blip:
                    # Don't increment — this isn't the meter's fault
                    continue
                # Meter missing this cycle
                miss_count = self._comm_miss.get(fdr_asset, 0) + 1
                self._comm_miss[fdr_asset] = miss_count
                if miss_count == 1:
                    # First miss — record timestamp, don't alert yet
                    self._comm_first[fdr_asset] = datetime.now().isoformat()
                elif miss_count == comm_miss_threshold:
                    # Exactly at threshold — fire the alert once
                    first_ts = self._comm_first.get(fdr_asset, datetime.now().isoformat())
                    feeder_name = (fdr_info.get("FeederName","") or
                                   fdr_info.get("Feeder","") or fdr_asset)
                    cv = {
                        "id":           f"{fdr_asset}_{VT_COMM}_{int(time.time()*1000)}",
                        "asset_key":    ak_comm,
                        "type":         VT_COMM,
                        "severity":     SEVERITY.get(VT_COMM,"HIGH"),
                        "AssetCode":    fdr_asset,
                        "Circle":       fdr_info.get("CircleName","")   or fdr_info.get("Circle",""),
                        "Division":     fdr_info.get("DivisionName","") or fdr_info.get("Division",""),
                        "Gss":          fdr_info.get("GssName","")      or fdr_info.get("Gss",""),
                        "Feeder":       feeder_name,
                        "FeederType":   fdr_info.get("FeederType",""),
                        "IsBusCoupler": False,
                        "FeederRating": fdr_info.get("FeederRating"),
                        "Vr":None,"Vy":None,"Vb":None,
                        "Ir":None,"Iy":None,"Ib":None,
                        "ApparentPower":None,
                        "value":        miss_count,
                        "limit":        comm_miss_threshold,
                        "detail":       (f"Meter data not received for "
                                         f"{miss_count} consecutive scan cycle(s). "
                                         f"First missing at {first_ts}. "
                                         f"Communication Down / Meter OFF / Feeder OFF."),
                        "timestamp":    datetime.now().isoformat(),
                        "acked":        False,
                        "notified_email": False,
                        "notified_wa":    False,
                        "_comm_first_missing": first_ts,
                        "_comm_miss_count":    miss_count,
                    }
                    if self.store.add(cv, self.cfg):
                        raw_viols.append(cv)
                        log.info(f"COMM_DOWN: {fdr_asset} ({feeder_name}) "
                                 f"missing for {miss_count} consecutive cycles — alert created")
                # else: already alerted (miss_count > threshold), still missing — no repeat
            else:
                # Meter is back — clear if was comm-down
                if self.store.is_active(ak_comm):
                    # Build normalization event
                    comm_dur = self.store.get_active_duration(ak_comm)
                    dur_str  = _fmt_dur_sec(comm_dur) if comm_dur else ""
                    first_ts = self._comm_first.get(fdr_asset, "")
                    feeder_name = (fdr_info.get("FeederName","") or
                                   fdr_info.get("Feeder","") or fdr_asset)
                    norm_v = {
                        "id":           f"{fdr_asset}_{VT_COMM_RST}_{int(time.time()*1000)}",
                        "asset_key":    f"{fdr_asset}_{VT_COMM_RST}",
                        "type":         VT_COMM_RST,
                        "severity":     "INFO",
                        "AssetCode":    fdr_asset,
                        "Circle":       fdr_info.get("CircleName","")   or fdr_info.get("Circle",""),
                        ...
                        "value":        0,
                        "limit":        comm_miss_threshold,
                        "detail":       (f"Meter communication restored after "
                                         f"{dur_str or 'unknown duration'}. "
                                         f"First missing was {first_ts}."),
                        "timestamp":    datetime.now().isoformat(),
                        "acked":        True,   # auto-ack like LOAD_RESTORED
                        "notified_email": False,
                        "notified_wa":    False,
                        "_comm_norm":           True,
                        "_comm_duration_str":   dur_str,
                    ...
                    self.store.clear_condition(fdr_asset, VT_COMM)
                    raw_viols.append(norm_v)
                    log.info(f"COMM_DOWN cleared: {fdr_asset} ({feeder_name}) "
                             f"communication restored after {dur_str or '?'}")
                # Reset counters regardless
                self._comm_miss.pop(fdr_asset, None)
                self._comm_first.pop(fdr_asset, None)
                # Also reset frozen tracking — meter was absent so last known
                # values are stale; don't let identical-but-old values trigger
                # a spurious frozen alert on the first cycle after return
                self._frozen_cycles.pop(fdr_asset, None)
                self._frozen_last.pop(fdr_asset, None)
                self._frozen_first.pop(fdr_asset, None)
        ...
        for v in self._bc.update(live_data, off_thr):
            ...
            if vtype == VT_RST:
                ...
                if self.store.add(v, self.cfg):
                    raw_viols.append(v)
            elif vtype in (VT_BCT, VT_BCR, VT_BCN):
                # FIX: these two are already inserted into the store INSIDE
                # BusCouplerDiversionDetector.update() itself, via
                # self._store_ref.add(v, None) — and self._store_ref IS
                # self.store (same AlertStore instance, see __init__). That
                # add() call already returned True (new alert) before this
                # violation was even put in the list we're iterating here.
                # Calling self.store.add(v, self.cfg) a SECOND time on the
                # exact same alert was the bug: VT_BCT is a STATEFUL/latched
                # type, so the second call sees the alert already marked
                # active (from the first call moments earlier) and returns
                # False — silently dropping it from raw_viols, so it never
                # reached _notify()/_notify_wa() and no WhatsApp/email ever
                # went out for "BC_TRIP_WHILE_DIVERTED", despite _wa_msg()
                # already having the exact requested format for it. Since
                # the BC detector only ever yields these AFTER a successful
                # add(), just trust that and queue for notification directly
                # — no second store write needed.
                raw_viols.append(v)
            else:
                if self.store.add(v, self.cfg):
                    raw_viols.append(v)
        ...
        if raw_viols:
            if self.notify_queue:
                # Fire-and-forget — enqueue() just writes to SQLite and
                # returns in <1ms; the queue worker's background thread
                # handles actual delivery (email, WhatsApp Selenium) with
                # retry/backoff, completely decoupled from this scan cycle.
                # This is the fix for the 2-minute interval being blown:
                # previously _notify() called wa.send() synchronously
                # inline here, which blocked the entire scan cycle for
                # ~77 seconds while Selenium drove the browser.
                self._notify(raw_viols)
            else:
                # Legacy path (notify_queue not wired in app.py yet):
                # fire in a daemon thread so at least the scan cycle
                # returns immediately even in the unqueued case —
                # thread-based sending isn't retry-safe but it's better
                # than blocking the cycle entirely.
                import threading as _th
                _th.Thread(target=self._notify, args=(raw_viols,),
                           daemon=True, name="notify-direct").start()

        # Retry pending delayed notifications in a separate daemon thread
        # so the delay-check DB query and any resulting sends never block
        # the scan cycle either.
        import threading as _th
        _th.Thread(target=self._retry_delayed, daemon=True,
                   name="retry-delayed").start()
    ...
    def _notify(self, violations: list):
        ...
        def should_notify(v):
            ...
            if v.get("_ljp_norm"):
                return True
            # PTF normalize/transient: always notify (no delay)
            if v.get("_ptf_norm"):
                return True
            # COMM_DOWN normalization: always notify (no delay)
            if v.get("_comm_norm"):
                return True
        ...
        # VT_RST/VT_BCT/VT_BCR always notify regardless of the configured
        # alert_types checkboxes — same reasoning as VT_RST already had:
        # a feeder genuinely losing its only remaining supply path (VT_BCT)
        # or that path being resolved (VT_BCR) is too important to silently
        # depend on whether these (newer) type strings happen to be ticked
        # in Settings — an operator who configured alert_types before these
        # types existed would otherwise never see them, with no obvious
        # reason why.
        ALWAYS_NOTIFY_TYPES = {VT_RST, VT_BCT, VT_BCR, VT_BCN, VT_COMM, VT_COMM_RST}

        e_viols = [v for v in violations
                   if should_notify(v) and (
                       v.get("type") in ALWAYS_NOTIFY_TYPES or
        ...
        def wa_to(v):
            """
            Global Contacts + Dedicated Area Contacts model:
            EVERY alert always reaches whatsapp.recipients (Global
            Contacts), and ADDITIONALLY reaches every Circle/Division
            contact-directory entry whose circle/division matches this
            violation — not exclusively one or the other. Previously this
            returned the FIRST matching area contact's numbers ONLY,
            falling back to global ONLY if nothing matched at all — so a
            circle that had its own dedicated contact group configured
            would silently stop reaching the global numbers entirely,
            which is what surprised you with the CHANDIPUR alert landing
            only in the Balasore-specific chat and nowhere else.
            """
            nos = list(self.cfg.get("whatsapp.recipients", []))
            for c in contacts:
            ...
            return list(dict.fromkeys(nos))  # dedupe, preserve order

        def email_extra_to(violations_batch):
            """
            Same Global + Dedicated model for email. email_mgr.py's
            send_violations() already always includes email.recipients
            (Global Contacts) — this just collects the ADDITIONAL
            Circle/Division-matched emails to union on top, across every
            violation in this batch (a consolidated email covers however
            many violations fired this cycle, so an area contact is
            included if ANY violation in the batch matches their circle/
            division — not just the first one).
            """
            extra = []
            for v in violations_batch:
                for c in contacts:
                    if not c.get("active", True): continue
                    if ((not c.get("circle")) or c["circle"].upper() == v.get("Circle","").upper()) and \
                       ((not c.get("division")) or c.get("division","").upper() in v.get("Division","").upper()):
                        extra += [e.strip() for e in (c.get("emails","")).replace(",","\n").split("\n") if e.strip()]
            return list(dict.fromkeys(extra))

        if self.cfg.get("email.enabled") and e_viols:
            extra_emails = email_extra_to(e_viols)
            if self.notify_queue:
                # Mark notified IMMEDIATELY on enqueue (optimistic marking)
                # — NOT after the queue worker confirms delivery. This is
                # the critical fix for duplicate sends: _retry_delayed()
                # runs every 2-min scan cycle and checks notified_wa/email
                # flags; if they're still False when retry fires (which
                # they always were before, since the queue worker only sets
                # them after actual delivery, which can take 77+ seconds),
                # _retry_delayed() re-queues the same alert every cycle,
                # resulting in one new WA send every 2 minutes forever.
                # Optimistic marking on enqueue stops that: once enqueued,
                # the flags are True, _retry_delayed() skips this alert.
                # The queue's retry/backoff handles real delivery failures;
                # a permanent failure is logged as ERROR — same outcome as
                # before the queue existed but now with retries first.
                for v in e_viols: self.store.mark_notified(v["id"], "email")
                self.notify_queue.enqueue_email(e_viols, extra_to=extra_emails)
            else:
                ok = self.email.send_violations(e_viols, extra_to=extra_emails)
                if ok:
                    for v in e_viols: self.store.mark_notified(v["id"],"email")
        ...
        if self.cfg.get("whatsapp.enabled") and w_viols:
            ...
            for nos, group in recipient_groups.items():
                ...
                for batch in batches:
                    if self.notify_queue:
                        # Same optimistic marking — see email comment above
                        for v in group: self.store.mark_notified(v["id"], "wa")
                        self.notify_queue.enqueue_wa(
                            list(nos), batch, [v["id"] for v in group if v.get("id")])
                    else:
    ...
    def _wa_msg(v: dict) -> str:
        ...
        ICONS  = {VT_OV:"🔴",VT_UV:"🟡",VT_OL:"🟠",VT_OFF:"⚫",
                  ...
                  VT_PTF:"⚠️",VT_LJP:"⛓️",VT_BCT:"⚫",VT_BCR:"✅",VT_BCN:"🔁",
                  VT_COMM:"📡", VT_COMM_RST:"✅"}
        LABELS = {VT_OV:"OV Alert",VT_UV:"UV Alert",VT_OL:"Overload Alert",
                  ...
                  VT_BCN:"BC Resumed Diversion",
                  VT_COMM:"Communication Down",
                  VT_COMM_RST:"Communication Restored"}
        icon  = ICONS.get(t,"⚡")
        label = LABELS.get(t, t)

        # BC's OWN current resumed while the originally-diverted feeder is
        # STILL off — the no-backup-path emergency has eased, but this is
        # explicitly NOT a "feeder restored" message (that's VT_BCR, which
        # only fires once the actual feeder itself comes back). Distinct
        # icon/wording on purpose so it can't be mistaken for the feeder
        # being fixed when it isn't yet.
        if t == VT_BCN:
            feeder  = v.get("_diverted_feeder_name") or v.get("AssetCode","")
            bc_name = v.get("Feeder") or v.get("AssetCode","")
            dur_str = v.get("_off_duration_str","")
            lines = [f"🔁 *TPNODL Bus Coupler Resumed*", "",
                     f"• Bus Coupler: {bc_name}",
                     f"• Area: {area}",
                     f"• GSS: {gss}",
                     f"• {det}"]
            if dur_str:
                lines.append(f"• ⏱ BC was tripped for: *{dur_str}*")
            lines.append(f"• ⚠️ Note: *{feeder}* is still OFF — only the Bus "
                         f"Coupler itself has recovered, tracked separately.")
        ...
        if t == VT_PTF and v.get("_ptf_norm"):
            feeder   = v.get("Feeder") or v.get("AssetCode","")
            is_false = v.get("_ptf_false", False)
            hdr  = "⚠️ *TPNODL PT Phase (Transient)*" if is_false else "✅ *TPNODL PT Phase Restored*"
            note = ("_Transient — voltage balance briefly disturbed, no fault_" if is_false
                    else "_Voltage balance restored — PT fuse/phase fault has cleared_")
            vr_v = v.get("Vr"); vy_v = v.get("Vy"); vb_v = v.get("Vb")
            lines = [hdr, note, "",
                     f"• Feeder: {feeder}",
                     f"• Area: {area}",
                     f"• GSS: {gss}"]
            if vr_v is not None and vy_v is not None and vb_v is not None:
                try:
                    lines.append(f"• Voltage (3-ph): Vr={float(vr_v):.3f}kV  Vy={float(vy_v):.3f}kV  Vb={float(vb_v):.3f}kV")
                except Exception:
                    pass
            lines += [f"• {det}",
                      f"• Time: {ts}"]
        ...
        if t in (VT_COMM, VT_COMM_RST):
            feeder = v.get("Feeder") or v.get("AssetCode","")
            first_missing = v.get("_comm_first_missing", ts)
            miss_count    = v.get("_comm_miss_count", 2)
            is_frozen     = v.get("_comm_frozen", False)
            if t == VT_COMM_RST:
                dur = v.get("_comm_duration_str","")
                kind_str = "Frozen data cleared" if is_frozen else "Communication restored"
                lines = [f"✅ *TPNODL Meter {kind_str}*", "",
                         f"• Feeder: {feeder}",
                         f"• Area: {area}",
                         f"• GSS: {gss}"]
                if dur:
                    lines.append(f"• ⏱ Was {'frozen' if is_frozen else 'down'} for: *{dur}*")
                if is_frozen:
                    vr_v=v.get("Vr"); vy_v=v.get("Vy"); vb_v=v.get("Vb")
                    ir_v=v.get("Ir"); iy_v=v.get("Iy"); ib_v=v.get("Ib")
                    if vr_v is not None:
                        lines.append(f"• Resumed values — Vr={float(vr_v):.3f} Vy={float(vy_v):.3f} Vb={float(vb_v):.3f} kV")
                        lines.append(f"  Ir={float(ir_v):.1f} Iy={float(iy_v):.1f} Ib={float(ib_v):.1f} A")
                lines += [f"• Time: {ts}"]
                return "\n".join(lines)
            else:
                if is_frozen:
                    ...
                    vr_v=v.get("Vr"); vy_v=v.get("Vy"); vb_v=v.get("Vb")
                    ir_v=v.get("Ir"); iy_v=v.get("Iy"); ib_v=v.get("Ib")
                    lines = ["🔒 *TPNODL Frozen Data / Meter OFF*", "",
                             f"• Feeder: {feeder}",
                             f"• Area: {area}",
                             f"• GSS: {gss}",
                             f"• No change in Vr/Vy/Vb/Ir/Iy/Ib for *{miss_count} consecutive cycles* (~{miss_count*2}min)"]
                    if vr_v is not None:
                        lines.append(f"• Stuck values — Vr={float(vr_v):.3f} Vy={float(vy_v):.3f} Vb={float(vb_v):.3f} kV")
                        lines.append(f"  Ir={float(ir_v):.1f} Iy={float(iy_v):.1f} Ib={float(ib_v):.1f} A")
                    lines += [f"• First frozen at: {first_missing}",
                              f"• Time: {ts}",
                              "_Note: Meter may be offline — SCADA is reporting cached values_"]
                    return "\n".join(lines)
                else:
                    # Absent-meter variant
                    lines = ["📡 *TPNODL Communication Down / Meter OFF*", "",
                             f"• Feeder: {feeder}",
                             f"• Area: {area}",
                             f"• GSS: {gss}",
                             f"• First data missing: {first_missing}",
                             f"• Missing scans: {miss_count} consecutive (≥{miss_count}× 2-min cycle)",
                             f"• Time: {ts}",
                             "_Note: Any real violation on this feeder may be undetected while communication is down_"]
                    return "\n".join(lines)
...
class BusCouplerDiversionDetector:
    ...
    def __init__(self):
        ...
        self.cfg             = None  # Config — set by ViolationDetector after construction
    ...
    def update(self, live_data: list, off_thr: float = 1.0) -> list:
        ...
        for gss, rows in by_gss.items():
            ...
            for bc in bcs:
                ...
                # ── BC_RESUMED_DIVERSION (previously missing) ───────────────
                # The BC's OWN current resuming is a DIFFERENT event from
                # the diverted feeder's own restoration below — confirmed
                # live (2026-06-25, GSS 220/132/33 KV GOLABANDHA): BC
                # TPN60093 was reading 37.4A (status DIV — actively
                # carrying the diversion again) while TELKOI FDR itself
                # still read 0.0A (still OFF) — yet BC_TRIP_WHILE_DIVERTED
                # sat active with no signal at all that the BC side had
                # recovered.
                #
                # ORIGINAL DESIGN (superseded by explicit instruction):
                # this used to NOT clear VT_BCT, on the reasoning that the
                # feeder being off is the real unresolved problem. That
                # has been explicitly overridden — VT_BCT now clears here.
                # This is safe: TELKOI FDR.'s own FEEDER_OFF and "Load
                # Diverted via BC" alerts stay separately active the
                # entire time (confirmed against live data — both were
                # still active at 127h/151h durations when this changed),
                # so the underlying outage is never silently lost — only
                # the BC's own redundant third flag on the same problem
                # goes away once the BC itself has recovered.
                #
                # Clearing here also makes the "fire once" guard automatic:
                # once VT_BCT is cleared, is_active() is False on every
                # later cycle, so this can't re-fire until a genuinely new
                # BC_TRIP_WHILE_DIVERTED incident creates a fresh alert.
                bct_ak_for_resume = f"{bac}_{VT_BCT}"
                if self._store_ref.is_active(bct_ak_for_resume) and bcimax > off_thr:
                    active_bct_for_resume = self._store_ref.get_active_alert(bct_ak_for_resume)
                    if active_bct_for_resume:
                        diverted_fac_r = active_bct_for_resume.get("_diverted_feeder", "")
                        diverted_name_r = active_bct_for_resume.get("_diverted_feeder_name", diverted_fac_r or bac)
                        dur_r = self._store_ref.get_active_duration(bct_ak_for_resume)
                        dur_str_r = _fmt_dur_sec(dur_r) if dur_r else ""
                        bcn_v = {
                            "id":        f"{bac}_{VT_BCN}_{int(time.time()*1000)}",
                            "asset_key": f"{bac}_{VT_BCN}",
                            "type":      VT_BCN,
                            "severity":  SEVERITY.get(VT_BCN,"INFO"),
                            "AssetCode": bac,
                            "Circle":    bc.get("Circle",""),
                            "Division":  bc.get("Division",""),
                            "Gss":       gss,
                            ...
                            "lim":       off_thr,
                            "detail":    (f"BC current restored: 0.0A→{bcimax:.1f}A — "
                                          f"BC is carrying the diversion again."),
                            "timestamp": now_iso,
                            "first_seen": now_iso,
                            "acked":     True,   # informational, auto-acknowledged
                            "notified_email": False,
                            "notified_wa":    False,
                            "_diverted_feeder":      diverted_fac_r,
                            "_diverted_feeder_name": diverted_name_r,
                            "_bc_asset":             bac,
                            "_off_duration_str":     dur_str_r,
                        }
                        if self._store_ref.add(bcn_v, None):
                            viols.append(bcn_v)
                            log.info(f"BC_RESUMED_DIVERSION: {bac} @ {gss} back to "
                                     f"{bcimax:.1f}A after {dur_str_r} — clearing "
                                     f"BC_TRIP_WHILE_DIVERTED (per explicit override); "
                                     f"{diverted_name_r}'s own FEEDER_OFF/Load Diverted "
                                     f"via BC alerts remain separately active")
                ...
                for fdr in fdrs:
                    ...
                    if fac in self._active and not feeder_now_off:
                        ...
                        if bc_reduced:
                            ...
                            # FIX — stuck-state safety net: this branch had NO
                            # timeout. If bc_reduced never evaluates True on any
                            # future cycle (the BC-load-share math can stay
                            # inconclusive indefinitely for some topologies —
                            # e.g. sibling feeders' f_before getting stale, or
                            # measurement noise that never quite clears the 50%
                            # tolerance band), `fac` stayed in self._active
                            # FOREVER even though the feeder is genuinely back
                            # ON and its FEEDER_OFF alert was ALREADY cleared by
                            # the main per-feeder loop moments earlier in this
                            # same cycle (that loop clears FEEDER_OFF
                            # unconditionally once imax > off_thr, deferring
                            # ONLY the LOAD_RESTORED creation to this detector).
                            # Net effect: a real restoration silently produces
                            # NO restoration record at all — not in the DB, not
                            # on the dashboard, not via WhatsApp/email — AND the
                            # feeder stays permanently excluded from the main
                            # loop's own restore path on every future trip too
                            # (since `asset in self._bc._active` keeps gating it
                            # there). This is exactly what happened to 33 KV
                            # BILEIPADA FDR.
                            #
                            # Track how long the feeder has been back ON while
                            # still stuck here, and force the restoration through
                            # after a bounded wait — a forced resolution after a
                            # few minutes is far better than an indefinite, silent
                            # loss of the restoration record.
                            if "_restored_since_ts" not in d:
                                d["_restored_since_ts"] = now
                            stuck_for_min = (now - d["_restored_since_ts"]) / 60
                            BC_RESTORE_FORCE_TIMEOUT_MIN = self.cfg.get(
                                "voltage.bc_restore_force_timeout_min", 10)
                            if stuck_for_min >= BC_RESTORE_FORCE_TIMEOUT_MIN:
                                d = self._active.pop(fac)
                                self._pending.pop(fac, None)
                                v = self._viol(VT_RST, fdr, d["bc"],
                                               d["f_before"], d["bc_after"], fimax,
                                               first_rise_ts=d.get("first_rise_ts"))
                                v["detail"] = (
                                    f"Feeder {fdr.get('Feeder',fac)} restored "
                                    f"({fimax:.1f}A). BC load did not conclusively "
                                    f"confirm reduction after {stuck_for_min:.0f}min "
                                    f"(BC={bcimax:.1f}A, expected remaining "
                                    f"{expected_remaining:.1f}A) — restoration "
                                    f"recorded anyway since the feeder's own "
                                    f"current has clearly normalized.")
                                viols.append(v)
                                log.warning(
                                    f"LOAD_RESTORED (BC diversion, FORCED after "
                                    f"{stuck_for_min:.0f}min timeout): {fac} @ {gss} "
                                    f"— feeder back {fimax:.1f}A but bc_reduced never "
                                    f"confirmed cleanly. Forcing restoration to avoid "
                                    f"permanently stuck BC-tracking state.")
                                self._stale_vals.pop(fac, None)
